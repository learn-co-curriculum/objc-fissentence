...
4. The `replacePunctuation:withPunctuation:` method should overwrite the string in the `punctuation` property string with the new `punctuation` argument string. This method should only attempt to make the change if both of the new `punctuation` argument strings passes validation.
